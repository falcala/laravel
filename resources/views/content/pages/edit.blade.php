          ...
          <div class="tab-panel p-3" id="tab-nav" style="display:none">
            ...
            <div class="mb-3">
              ...
              <div class="d-flex gap-2 flex-wrap">
                @foreach(['normal' => ['bx-menu','Normal'],'sticky' => ['bx-pin','Sticky'],'fixed' => ['bx-dock-top','Fixed']] as $pos => [$icon, $label])
                <label class="border rounded px-2 py-1 d-flex align-items-center gap-1 cursor-pointer small
                              {{ ($page->nav_position ?? 'normal') === $pos ? 'border-primary text-primary bg-label-primary' : '' }}"
                       style="cursor:pointer" id="nav-pos-label-{{ $pos }}">
                  <input type="radio" name="nav_position" value="{{ $pos }}" class="d-none nav-pos-radio"
                         {{ ($page->nav_position ?? 'normal') === $pos ? 'checked' : '' }}>
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  // ── Nav menu builder ──────────────────────────────────────────────
  var navItems = @json($page->nav_items ?? []);
